...
export function useRealmApi(realmUser) {
    ...
    const callRealmApi = useCallback(
        async (functionName, functionArgs) => {
            if (!realmUser) return { error: { message: 'You need to be logged in to continue.' } };

            const isOffline = !isConnected || !isInternetReachable;
            if (isOffline) return { error: { message: 'Please connect to the Internet to continue.' } };
            ...
            // via our change listeners implemented in each provider (see /app/providers).
